...
##########################################################################################
# MAXITER_stage_2 = 10
global MAXITER_single_stage
MAXITER_single_stage = 15
n_opts = 15
more_iter_each_opt = 3
...
nphi_VMEC = 26
ntheta_VMEC = 26
coils_objective_weight = 2e+2
diff_method = "forward"
quasisymmetry_target_surfaces = [0, 0.2, 0.4, 0.6, 0.8, 1]
finite_difference_abs_step = 5e-5
finite_difference_rel_step = 5e-3
JACOBIAN_THRESHOLD = 1e5
...
LENGTH_THRESHOLD *= R0_coils
CC_THRESHOLD *= R0_coils
CURVATURE_THRESHOLD /= R0_coils
MSC_THRESHOLD /= R0_coils
...
def output_dofs_to_csv(vmec_dofs, coils_dofs, iota, qs_res, aspect):
    keys=np.concatenate([[f'vmec({i})' for i, dof in enumerate(vmec_dofs)],[f'coils({i})' for i, dof in enumerate(coils_dofs)], ['iota'], ['qs_res'], ['aspect']])
    values=np.concatenate((np.concatenate((vmec_dofs, coils_dofs)), [iota], [qs_res], [aspect]))
...
def fun(dofs, prob_jacobian=None, info={'Nfeval': 0}):
    ...
    if J > JACOBIAN_THRESHOLD or np.isnan(J):
        ...
        if (info['Nfeval'] > MAXITER_single_stage):
            proc0_print(f"Reached maximum number of iterations: {MAXITER_single_stage}")
            return J, [0] * len(dofs)
    ...
    try: output_dofs_to_csv(prob.x, JF.x, vmec.mean_iota(), qs.total(), vmec.aspect())
    except Exception as e: pass#proc0_print(f"  Exception caught during output_dofs_to_csv: {e}")
...
objective_tuple = [(vmec.aspect, aspect_ratio_target, 1), (vmec.mean_iota, 0.41, 1e3)]#, (qs.residuals, 0, 1e-1)]
...
for i in range(n_opts):
    JF.x = dofs[:-number_vmec_dofs]*1.0001
    prob.x = dofs[-number_vmec_dofs:]*1.0001
    bs.set_points(surf.gamma().reshape((-1, 3)))
    ...
    prob.x = dofs[-number_vmec_dofs:]
    bs.set_points(surf.gamma().reshape((-1, 3)))
    proc0_print(f'  Performing single stage optimization again with ~{MAXITER_single_stage} iterations')
    with MPIFiniteDifference(prob.objective, mpi, diff_method=diff_method, abs_step=1e-7, rel_step=0) as prob_jacobian:
        if mpi.proc0_world:
            res = minimize(fun, dofs, args=(prob_jacobian, {'Nfeval': 0}), jac=True, method='BFGS', options={'maxiter': MAXITER_single_stage}, tol=1e-5)
            dofs = res.x
    mpi.comm_world.Bcast(dofs, root=0)
    
    MAXITER_single_stage+=more_iter_each_opt
Bbs = bs.B().reshape((nphi_VMEC, ntheta_VMEC, 3))
BdotN_surf = np.sum(Bbs * surf.unitnormal(), axis=2)
if comm_world.rank == 0:
